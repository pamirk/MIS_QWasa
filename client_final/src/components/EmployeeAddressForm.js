...
export default class EmployeeAddressForm extends Component {
    ...
    postData = () => {
        ...
        if (!(this.id && this.current_address.value && this.current_address.value && this.postal_code.value && this.phone_number.value
            ...
            Util.showErrorMessage('Please provide all the Fields', 5);
        } else {
            fetch('http://localhost:3003/api/employee_create_address', {
                method: 'POST',
                headers: {
                ...
                .then(data => {
                    ...
                    console.log(data);
                    if (data.status === 500) {
                        Util.showErrorMessage('Server Error ', 5);
                    } else if (data.status === 200) {
                        Util.showSuccessMessage('Successfully Added', 5);
                        this.props.history.push(`/employeeAddressHome/${this.id}`);
                    }
                })
