...
export default class EmployeeDesignationForm extends Component {
    ...
    postData = () => {
        ...
        if (!(this.id && this.menu.value && this.Des_menu.value && this.des_order_date.value
            ...
            axios.post('http://localhost:3003/api/create_employee_designation', fd)
                .then(d => {
                    const data = d.data;
                    console.log(data);
                    this.setState({
                    ...
                    if (data.err) {
                        Util.showErrorMessage(data.err, 5);
                    } else if (data.status === 200) {
                        Util.showSuccessMessage('Successfully Added Employee Designation', 5);
                        this.props.history.push(`/employee_designationHome/${this.id}`);
                    }
